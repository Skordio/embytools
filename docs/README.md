# embytools documentation

Verbose reference docs for embytools, a personal CLI for managing an Emby
server. For installation and a quickstart, see the [project README](../README.md).

## Sections

- **[Commands reference](commands/README.md)** — every command, its arguments,
  options, and examples, grouped by area:
  - [users](commands/users.md)
  - [channels](commands/channels.md) — favorites: list, copy, export/import
  - [channels numbers](commands/channels-numbers.md) — channel numbering and schemes
  - [sessions](commands/sessions.md) — active sessions and playback control

More sections (configuration, schemes & plugins, the snapshot/recovery model)
will be added here over time.
